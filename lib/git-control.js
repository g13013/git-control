...
Git.prototype = {
  ...
  saveRestore: function () {
    ...
    return execSequence([['stash', 'pop'], ['stash', 'drop']], this).then(function () {
      self.saved = false;
    });
  },
    ...
    return this.info.then(function (info) {
      ...
      if (full) {
        ...
        for (key in info.submodules) {
          sub[key] = {};
          sub[key]['path'] = info.submodules[key].repo.path;
          sub[key]['currentRev'] = info.submodules[key].repo.info.currentRev;
        }
      }
