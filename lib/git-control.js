...
var Git = function (options) {
  ...
  this.define('workTree', function (value) {
    workTree = value;
    return workTree;
  },function () {
  ...
  this.define('name', function (value) {
    name = value;
    return name;
  }, function () {
    return name || path.basename( path.resolve(self.path) );
  });

  this.define('gitDir', function (value) {
    gitDir = value;
    return gitDir;
  }, function () {
    return gitDir || (self.path + '/.git');
  });
  ...
  self.info = {
    then: function (successHandler, errorHandler) {
      if (!options.isSubmodule) {
        return self.getInfo(true).then(successHandler, errorHandler);
      }
      return options.parent.submodule.initUpdate().then(function () {
        return self.getInfo(true).then(successHandler, errorHandler);
      });
    }
  };
...
Git.prototype = {
  ...
  submodule: function (command) {
    ...
    self.submodule.initUpdate = function () {
      ...
      return execSequence([['exec', 'submodule update --init']], self).then(function () {
        self.isInititalized = true;
        self.isUpdated = true;
      });
    };
    ...
    return this.exec('submodule status').then(function (out) {
      var submodule,
          states = [],
          register = function (sha, path, revision, state) {
            var repo,
                isInititalized = state !== '-',
                isUpdated = state !== '+',
                hasConflicts = state === 'U',
                subName = path.split('/').pop(),
            ...
            subObj = subObj || {
              repo: new Git({
                path: path,
                parent: self,
                gitDir: self.gitDir + '/modules/' + path,
                isSubmodule: true,
